...
def subsequent_outcome(df: pd.DataFrame,
    ...
    :return: Returns the dataframe with 7 new columns that define the feasible date a subsequent outcome of each
            class can occur
    """
...
def process_spacing(df: pd.DataFrame,
    ...
    :return: Returns the original pandas dataframe with the pregnancy start window (Max term and Min term) and
    the subsequent event dates added in.
    """
    ...
    df['Subsequent_Start_Date'] = pd.to_datetime(df[admit_col]) + pd.to_timedelta(df[SUBSEQUENT], unit='d')
...
def spacing(df: pd.DataFrame,
    ...
    :return: Returns the original dataframe with the spacing information added defining the pregnancy start window
    for the current outcome as well as the spacing to the next pregnancy start and next pregnancy outcome by class.
    The subsequent pregnancy data is used to validate pregnancy outcomes as possible longitudinally.
    """
...
def validate_outcomes(df: pd.DataFrame,
                      ...
                      encounter_col: str):
    """
    Receives a df sliced by the unique patient identifier. Iterates over each encounter within an individual
    and validates the outcome types. Outcome types are processed according to the hierarchical order to determine
    validity.
    ...
    :return: Returns the original pandas dataframe with the outcome_valid column appended and completed. 1 indicates
            ...
            if df.iloc[idx][outcome_col] == outcome:
                ...
                if not df.iloc[idx]['outcome_valid']:
                    ...
                    if len(df_valid.index) == 0:
                        ...
                        # If other valid encounters exist, add the current encounter to that timeline and sort it
                        ...
                        current_index = df_valid[df_valid[encounter_col] == df[encounter_col].iloc[idx]].index.values
                        if len(current_index) == 1:
                            ...
                            if current_index[0] == 0:  # First in the list
                                ...
                                valid_before, event_before = next_event_valid(df_valid.iloc[current_index[0] - 1],
                                                                              df_valid.iloc[current_index[0]],
                                                                              base=1)
                                valid_after, event_after = next_event_valid(df_valid.iloc[current_index[0]],
...
def set_preg_window(df: pd.DataFrame):
    """
    Utility function to convert the spacing data to a date for the pregnancy window start and end

    :param df: Pandas dataframe containing classified pregnancies with an event_date, max_term, and min_term present

    :return: Returns the original dataframe with the start_window and end_window dates appended to the columns
    """
    df = df.assign(start_window=lambda x: calc_preg_window(x[EVENT_DATE], pd.to_timedelta(x[MAX_TERM], unit='d')),
                   end_window=lambda x: calc_preg_window(x[EVENT_DATE], pd.to_timedelta(x[MIN_TERM], unit='d')))
...
def calc_preg_window(date, offset):
    """
    Utility function to calculate a date from a base date and an offset. Function is designed assuming the offset
    refers to the past. Future dates should use a negative offset.
