...
def get_score(df: pd.DataFrame,
    ...
    :return: Pandas dataframe with total comorbidity score for the chosen method by patient and pregnancy
    """
...
def bateman_score(df: pd.DataFrame,
    ...
    if patient_col not in df.columns:
        raise ValueError(f'Patient identifier column {patient_col} is not in the provided dataframe.')
    ...
    # Collect the data that matches the search terms, reference the gestation hypertension indicator directly
    ...
    # Identify the gestational hypertension rows for which an exclusion is present for the same patient and pregnancy
...
def leonard_score(df: pd.DataFrame,
    ...
    # Sum up the scores
    output = df[[patient_col, pregnancy_col, indicator_col, score_col[0], score_col[1]]].fillna(0).drop_duplicates()
